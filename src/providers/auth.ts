...
export const authProvider: AuthProvider = {
  login: async ({ username, password }) => {
    try {
      ...
      const response = await axios.post("https://app.cmrsapp.site/user-management/api/auth/login", {
        username,
        password,
      }, {
        headers: {
      ...
      try {
        ...
        if (isAdmin || decodedToken.roles.includes("staff")) {
          const staffResponse = await axios.get(`https://app.cmrsapp.site/user-management/api/staff/${userId}`, {
            headers: {
              "accept": "*/*",
          ...
          // Fetch student data
          const studentResponse = await axios.get(`https://app.cmrsapp.site/user-management/api/students/secure/${userId}`, {
            headers: {
              "accept": "*/*",
              "Authorization": `Bearer ${response.data.accessToken}`
            }
