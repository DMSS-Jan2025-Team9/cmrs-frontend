...
export const CourseClassList: React.FC = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch("https://app.cmrsapp.site/course-management/api/courses/getActiveCourses")
            .then((res) => res.json())
            .then((data: Course[]) => {
                setCourses(data);
                setLoading(false);
    ...
    const fetchClassesForCourse = async (courseId: number) => {
        ...
        try {
            const res = await fetch(`https://app.cmrsapp.site/course-management/api/classSchedule?courseId=${courseId}`);
